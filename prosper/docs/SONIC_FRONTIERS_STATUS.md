...
| The intro's 3.3 fps is GPU-bound | **No.** `gpu-device` is 272 ms of a 4760 ms window (5.4%), the main thread is 2.1% on `drm_syncobj_wait`, and process CPU is 1.01 of 32 cores. The GPU is idle waiting for the host. #3405 |
...
| The per-dispatch `linear_seed` allocation is the cost | **No.** Implemented (a `thread_local` vector reused across dispatches, removing the allocation entirely), measured, and **reverted**: flips 3.36/3.19/3.39 against a 3.39 baseline, and every symbol in the perf profile unchanged within noise. `linear_seed` is only reached on the `atomic_image` and `zero_padded_tail` branches and nothing established the hot program takes either. #3405 |
| The compute image cache is thrashing | **No.** `PROSPER_COMPUTE_IMAGE_CACHE_MB=8192` left the hot program's setup at 1011-1110 ms against 1147 ms. Uploads were already being skipped: `upload-skipped=1` on all 19 persistent 33 MB storage images. #3405 |
| The compute memory pool is too small | **No.** `PROSPER_COMPUTE_MEMORY_POOL_MB=8192` left setup at 1130 ms and flips at 3.99/s. Note the field `allocation-reused` in `[compute-image]` is `bi.forced_seed_allocation_reused`, a seed flag — **not** a pool hit rate; reading it as one produced a wrong conclusion first time round. #3405 |
| Staging is write-combined, so the copies are slow | **No.** `host_memory_type` (`live_compute.cpp:3233`) prefers `HOST_CACHED`, and `persistent_ds_transfer_buffer` deliberately selects it for readbacks. #3405 |
| `upload_mapping.unmap()` costs the frame | **No.** A dedicated sub-timer recorded **zero** records above 0.5 ms across a full run. #3405 |
...
| An FMV always presents new content, so caching conversions cannot help | **No.** Fingerprinting the conversion source over a 130 s run: one 33 MB plane was converted **497 times with exactly 1 distinct content** (99.8% redundant), another 497 times with 98 distinct (80.3%). **90% of conversions reconvert bytes already converted.** #3407 |
| Pooling the readback destination buffer is worth doing | **No, on its own.** A bisection splits a color-target readback into setup 121.6 ms (18.4%) and the `output.assign` copy 537.7 ms (81.6%), so pooling targets the smaller share. Implemented and measured anyway: flips overlapped (4.59/4.79/5.03 vs 5.04/4.98/4.98) and readback was **worse by median** (1195 vs 1261 ms). Reverted. It only pays as part of the pooled-deleter design in #3407, which also unlocks a memset-free parallel copy. #3407 |
