...
| Pooling the readback destination buffer is worth doing | **UNDECIDED, not falsified — and it was reverted anyway.** Recorded here so nobody re-runs the same three-per-arm A/B expecting it to settle. A bisection splits a colour-target readback into setup **121.6 ms (18.4%)** and the `output.assign` copy **537.7 ms (81.6%)**, so pooling attacks the smaller share; that part is solid. The A/B is not: **baseline** flips 4.59/4.79/5.03 (median 4.79) against **pooled** 5.04/4.98/4.98 (median 4.98), i.e. the primary metric *favours* pooling, while the `readback` component timer says the opposite (baseline 1195 ms, pooled **1261 ms**). Two instruments disagree at n=3 per arm and no pooled-buffer reuse counter was read. It was reverted on the component timer plus the 18.4% ceiling, which is a judgement call, **not** a falsification. What would settle it: the pooled-deleter design in #3407, which makes the destination storage stable enough to drop the `assign` memset and copy in parallel — i.e. it attacks the 81.6%. Take both or neither. #3407 |
