...
| A UINT compute descriptor can bind a renderer's UNORM colour target in place, since they are the same bytes | **No — it renders WRONG COLOURS, and every automated instrument said it worked.** Implemented (`MUTABLE_FORMAT` on renderer targets + widening `direct_sampled_rtt_compatible` to admit `Uint8`x4 over `Rgba8Unorm`) and the plumbing did exactly what it promised: **546 in-place binds, zero `[rtt-remat]` readbacks, zero CPU conversions**, and a three-per-arm A/B with no overlap (distinct median 1.00/1.00/1.50 → 4.00/5.00/3.50). Then the project owner watched it: the SEGA logo, intro FMV, third-party logos and auto-save notice were all corrupted — *"everything except black"*, the signature of a value-space error, since 0 maps to 0 under either reading. **The premise is true of STORAGE and false of SAMPLING**: a UNORM sampler returns `byte/255` in [0,1], a UINT view hands the shader the integer 0-255, and the CPU arm being bypassed (`upload[t * n + c] = …` in the sampled conversion chain) was doing real work. **The lesson is the instrument, not the format**: six A/B runs, a mechanism check and an absolute measurement all counted FRAMES, and corrupted frames are just as distinct as correct ones — nothing in the harness was pointed at correctness at all. Reopening this needs a value-correct path (GPU-side conversion, or a UNORM view plus a shader-side scale), not the switch. `PROSPER_RTT_MUTABLE_FORMAT` is opt-in and default OFF, kept only so the falsifying A/B stays reproducible. #3407 |
| The intro's remaining cost is one dominant component | **No — it is split, and the F8 report says so itself** (*"inconclusive — no component reaches the 40% evidence threshold"*). Windowed, `--present-mode immediate`, GPU present adopted, 4.76 s: `readback` 1181.9 ms (24.8%), `renderer-resource` 1166.2 ms (24.5%, of which rtt materialization 711.1), `compute` 931.7 ms (19.6%), `gpu-wait` 446.1, **`gpu-device` 353.6 ms (7.4%)**. Guest flips 5.04/s, process CPU **1.68 cores** — up from 1.01 before #3408, so the work is no longer purely serial. Two components must be attacked, not one. #3415 |
| An offscreen (`SDL_VIDEODRIVER=offscreen`) capture measures this title's readback cost | **No — it measures the harness.** The F8 report detects and says so: without GPU present the frontend copies every scanout frame to the CPU, and that readback *"is the harness, not the title"*. A whole session's measurements were taken this way before the report's own warning caught it. **Measure through a real window with `--present-mode immediate`**; the report tells you which regime it was in, so check that line before quoting a readback number. #3415 |
